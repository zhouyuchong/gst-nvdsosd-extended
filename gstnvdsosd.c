...
gst_nvds_osd_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  ...
  // printf ("PUSHING CONTEXT %s, draw mask: %d\n", context_name, nvdsosd->draw_mask);
  NvDsMetaList *l = NULL;
  ...
  if (rect_cnt != 0 && nvdsosd->enable_mosaic) {
    nvdsosd->frame_rect_params->num_rects = nvdsosd->num_rect;
    nvdsosd->frame_rect_params->rect_params_list = nvdsosd->rect_params;
    /** Use of buf_ptr is deprecated, use 'nvdsosd->frame_rect_params->surf' instead */
    nvdsosd->frame_rect_params->buf_ptr = NULL;
...
static gboolean dump_info(GstNvDsOsd *nvdsosd) {   
    ...
    if (!surface) {
      ...
      GST_WARNING_OBJECT(nvdsosd, "Surface is NULL, skip mosaic");
      return -1;
    }

    NvBufSurfaceParams *surfaceList = surface->surfaceList;
    int num_rect = frame_rect_params->num_rects;
    NvOSD_RectParams *rect_params_list = frame_rect_params->rect_params_list;
    float *bbox = (float *)malloc(num_rect * 4 * sizeof(float));
    for (int i = 0; i < num_rect; i++){
      // int left = rect_params_list[i].left;
      // int top = rect_params_list[i].top;
      // int width = rect_params_list[i].width;
      // int height = rect_params_list[i].height;

      bbox[i * 4 + 0] = (float)rect_params_list[i].left;
      bbox[i * 4 + 1] = (float)rect_params_list[i].top;
      bbox[i * 4 + 2] = (float)rect_params_list[i].width;
      bbox[i * 4 + 3] = (float)rect_params_list[i].height;
    }
    for(int i= 0; i < surface->numFilled; i++) {
      // printf("  surface %d: color format: %d\n", i, surfaceList[i].colorFormat);
      // GST_WARNING_OBJECT(, "Landmark info is invalid, skip alignment");
      gint frame_width = surfaceList[i].width;
      gint frame_height = surfaceList[i].height;
      size_t frame_size = surfaceList[i].dataSize;
      // printf("  surface width: %d, height: %d\n", frame_width, frame_height);

      applyMosaicC(surfaceList[i].dataPtr, frame_width, frame_height, surfaceList[i].pitch, bbox, num_rect, 1);

      
      // cudaMallocHost(&local_img_ptr, frame_size);
      // cudaMemcpy(local_img_ptr, surfaceList[i].dataPtr, frame_size, cudaMemcpyDeviceToHost);
      
      // char filename[256];
      // sprintf(filename, "frame_%d_%d.png", frame_num, i);
      // int ret = stbi_write_png(filename, frame_width, frame_height, 4, (unsigned char*)local_img_ptr, surfaceList[i].pitch);
      // printf("write png ret: %d\n", ret);
    }
    // cudaFreeHost(local_img_ptr);
    return 0;
...
gst_nvds_osd_class_init (GstNvDsOsdClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SHOW_MOSAIC,
      g_param_spec_boolean ("enable-mosaic", "text",
          "Whether to enable mosaic", FALSE, G_PARAM_READWRITE));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SHOW_MOSAIC:
      nvdsosd->enable_mosaic = g_value_get_boolean (value);
      break;
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SHOW_MOSAIC:
      g_value_set_boolean (value, nvdsosd->enable_mosaic);
      break;
    case PROP_CLOCK_FONT:
